...
class AddInfo(object):
    ...
    def _lookup(self, key, protein, ligand):
        info = self._info
        p = info[info['protein'] == protein]
        l = p[p['ligand'] == ligand]
        v = list(l[key])[0]
        return v
    ...
    def __call__(self, x):
        print(x['id'])
        ligand, _, protein = x['id'].split('_')
        print('Looking up info for'+protein+' with '+ligand)
        for key in ['ligand_name', 'active_struc', 'inactive_struc',
                    'label', 'Dgscore', 'gscoreA', 'gscoreI',
                    'SMILES', 'Scaffold','Scaffold_Counts']:
            print(key)
            x[key] = self._lookup(key, protein, ligand)
        return x
...
def prepare(csv_file_path, input_file_path, output_root, no_gen, split, train_txt, val_txt, test_txt,
            score_path):
