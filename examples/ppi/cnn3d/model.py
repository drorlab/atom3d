...
class CNN3D_PPI(nn.Module):
    ...
    def forward(self, x_left, x_right):
        processed_left = self.base_net(x_left)
        processed_right = self.base_net(x_right)
        x = torch.cat([processed_left, processed_right], 1)
        x = self.top_net(x)
